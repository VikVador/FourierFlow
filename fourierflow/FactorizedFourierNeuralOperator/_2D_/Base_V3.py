...
class SpectralConv2d(nn.Module):
    ...
    def forward_fourier(self, x):
        ...
        if self.mode == 'full':
            out_ft[:, :, :, self.modes_y[0]:self.modes_y[1]] = torch.einsum("bixy,ioy->boxy",
                x_fty[:, :, :, self.modes_y[0]:self.modes_y[1]], torch.view_as_complex(self.fourier_weight[1]))
        ...
        if self.mode == 'full':
            out_ft[:, :, self.modes_x[0]:self.modes_x[1], :] = torch.einsum("bixy,iox->boxy",
                x_ftx[:, :, self.modes_x[0]:self.modes_x[1], :],
...
class FFNO(nn.Module):
    def __init__(self, input_dim,
        ...
        Documentation
        -------------
        Slightly modified FFNO operator now working as a pass-band filter !
        ...
        assert len(modes_x) == 2 & len(modes_y) == 2, \
            "(FFNO2D V3 - Module) modes should be a tuple of 2D containing the min and max modes defining the pass-band filter"
